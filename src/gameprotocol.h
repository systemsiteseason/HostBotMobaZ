...
public:
  ...
  std::vector<uint8_t> SEND_W3GS_GAMEINFO(uint8_t war3Version, const std::vector<uint8_t>& mapGameType, const std::vector<uint8_t>& mapFlags, const std::vector<uint8_t>& mapWidth, const std::vector<uint8_t>& mapHeight, const std::string& gameName, const std::string& hostName, uint32_t upTime, const std::string& mapPath, const std::vector<uint8_t>& mapCRC, uint32_t slotsTotal, uint32_t slotsOpen, uint32_t numPlayers, uint16_t port, uint32_t hostCounter, uint32_t entryKey);
  std::vector<uint8_t> SEND_W3GS_CREATEGAME(uint8_t war3Version);
  std::vector<uint8_t> SEND_W3GS_REFRESHGAME(uint32_t entryKey, uint32_t players, uint32_t playerSlots);
  std::vector<uint8_t> SEND_W3GS_DECREATEGAME(uint32_t entryKey, uint16_t port);
