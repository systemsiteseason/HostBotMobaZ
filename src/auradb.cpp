...
CDBBan* CAuraDB::BanCheckLocal(const string& owner, string user)
{
  CDBBan* Ban = nullptr;
  transform(begin(user), end(user), begin(user), ::tolower);

  if (!BanCheckStmt)
    m_DB->Prepare("SELECT name, date, reason FROM bans WHERE admin=? AND name=?", &BanCheckStmt);

  if (BanCheckStmt)
  {
    sqlite3_bind_text(static_cast<sqlite3_stmt*>(BanCheckStmt), 1, owner.c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(static_cast<sqlite3_stmt*>(BanCheckStmt), 2, user.c_str(), -1, SQLITE_TRANSIENT);
    ...
    if (RC == SQLITE_ROW)
    {
      if (sqlite3_column_count(static_cast<sqlite3_stmt*>(BanCheckStmt)) == 3)
      {
        string Name   = string((char*)sqlite3_column_text(static_cast<sqlite3_stmt*>(BanCheckStmt), 0));
        string Date   = string((char*)sqlite3_column_text(static_cast<sqlite3_stmt*>(BanCheckStmt), 1));
        string Reason = string((char*)sqlite3_column_text(static_cast<sqlite3_stmt*>(BanCheckStmt), 2));

        Ban = new CDBBan("",Name, Date, owner, Reason);
      }
      else
        Print("[SQLITE3] error checking ban [" + user + "] - row doesn't have 4 columns");
    }
    else if (RC == SQLITE_ERROR)
      Print("[SQLITE3] error checking ban [" + user + "] - " + m_DB->GetError());

    m_DB->Reset(BanCheckStmt);
  }
  else
    Print("[SQLITE3] prepare error checking ban [" + user + "] - " + m_DB->GetError());
...
bool CAuraDB::BanAddLocal(string user, const string& admin, const string& reason)
{
  bool          Success = false;
  sqlite3_stmt* Statement;
  transform(begin(user), end(user), begin(user), ::tolower);
  m_DB->Prepare("INSERT INTO bans (server, name, date, admin, reason ) VALUES ('mobaz', ?, date('now'), ?, ? )", reinterpret_cast<void**>(&Statement));

  if (Statement)
  {
    sqlite3_bind_text(Statement, 1, user.c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(Statement, 2, admin.c_str(), -1, SQLITE_TRANSIENT);
    ...
    if (RC == SQLITE_DONE)
      ...
      Print("[SQLITE3] error adding ban [" + user + " : " + admin + " : " + reason + "] - " + m_DB->GetError());

    m_DB->Finalize(Statement);
  }
  else
    Print("[SQLITE3] prepare error adding ban [" + user + " : " + admin + " : " + reason + "] - " + m_DB->GetError());
