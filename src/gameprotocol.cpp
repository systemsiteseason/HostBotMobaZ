...
std::vector<uint8_t> CGameProtocol::SEND_W3GS_GAMEINFO(uint8_t war3Version, const std::vector<uint8_t>& mapGameType, const std::vector<uint8_t>& mapFlags, const std::vector<uint8_t>& mapWidth, const std::vector<uint8_t>& mapHeight, const string& gameName, const string& hostName, uint32_t upTime, const string& mapPath, const std::vector<uint8_t>& mapCRC, uint32_t slotsTotal, uint32_t slotsOpen, uint32_t numPlayers, uint16_t port, uint32_t hostCounter, uint32_t entryKey)
{
  if (mapGameType.size() == 4 && mapFlags.size() == 4 && mapWidth.size() == 2 && mapHeight.size() == 2 && !gameName.empty() && !hostName.empty() && !mapPath.empty() && mapCRC.size() == 4)
  {
    ...
    AppendByteArray(packet, numPlayers, false);  // ???
...
std::vector<uint8_t> CGameProtocol::SEND_W3GS_REFRESHGAME(uint32_t entryKey, uint32_t players, uint32_t playerSlots)
{
  std::vector<uint8_t> packet = {W3GS_HEADER_CONSTANT, W3GS_REFRESHGAME, 16, 0};
  AppendByteArray(packet, entryKey, false);     // entrykey
  AppendByteArray(packet, players, false);     // Players
  AppendByteArray(packet, playerSlots, false); // Player Slots
  return packet;
}

std::vector<uint8_t> CGameProtocol::SEND_W3GS_DECREATEGAME(uint32_t entryKey, uint16_t port)
{
  std::vector<uint8_t> packet = {W3GS_HEADER_CONSTANT, W3GS_DECREATEGAME, 10, 0};
  AppendByteArray(packet, entryKey, false); // entrykey
  AppendByteArray(packet, port, false); // entrykey
  return packet;
}
