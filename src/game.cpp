...
bool CGame::Update(void* fd, void* send_fd)
{
  ...
  if (Time - m_LastPingTime >= 5)
  {
    ...
    if (!m_CountDownStarted)
    {
      ...
      //m_Aura->m_UDPSocket->Broadcast(6112, m_Protocol->SEND_W3GS_GAMEINFO(m_Aura->m_LANWar3Version, CreateByteArray(static_cast<uint32_t>(MAPGAMETYPE_UNKNOWN0), false), m_Map->GetMapGameFlags(), m_Map->GetMapWidth(), m_Map->GetMapHeight(), m_GameName, m_OwnerName, 0, m_Map->GetMapPath(), m_Map->GetMapCRC(), MAX_SLOTS, MAX_SLOTS, MAX_SLOTS - GetSlotsOpen(), m_HostPort, m_HostCounter & 0x0FFFFFFF, m_EntryKey));
      m_Aura->m_Broadcast->SendData(m_Protocol->SEND_W3GS_GAMEINFO(m_Aura->m_LANWar3Version, CreateByteArray(static_cast<uint32_t>(MAPGAMETYPE_UNKNOWN0), false), m_Map->GetMapGameFlags(), m_Map->GetMapWidth(), m_Map->GetMapHeight(), m_GameName, m_OwnerName, 0, m_Map->GetMapPath(), m_Map->GetMapCRC(), MAX_SLOTS, MAX_SLOTS, MAX_SLOTS - GetSlotsOpen(), m_HostPort, m_HostCounter & 0x0FFFFFFF, m_EntryKey));
    }
...
void CGame::SendCancel()
{
  m_Aura->m_Broadcast->SendData(m_Protocol->SEND_W3GS_DECREATEGAME(m_EntryKey, m_HostPort));
}
...
void CGame::EventPlayerJoined(CPotentialPlayer* potential, CIncomingJoinPlayer* joinPlayer)
{
  ...
  CDBBan* Ban = m_Aura->m_DB->BanCheckLocal(m_OwnerName, joinPlayer->GetName());
  ...
  if (Ban)
  {
    ...
    if (m_IgnoredNames.find(joinPlayer->GetName()) == end(m_IgnoredNames))
    {
      ...
      SendAllChat("User [" + Ban->GetName() + "] was banned on [" + Ban->GetDate() + "] by [" + Ban->GetAdmin() + "]");
...
bool CGame::EventPlayerBotCommand(CGamePlayer* player, string& command, string& payload)
{
    ...
    if (!m_Locked || RootAdminCheck || IsOwner(User))
    {
      ...
      switch (CommandHash)
      {
        ...
        */
        ...
        case HashCode("k"):
        {
          if (Payload.empty() || m_GameLoaded || m_GameLoading)
            break;
          if (Payload == User)
          {
            SendChat(player, "You cann't kick yourself!");
            break;
          }
        ...
        case HashCode("unban"):
        {
          ...
          if (m_Aura->m_DB->BanRemove(Payload))
            SendChat(player, "Unbanned user [" + Payload + "]");
          else
            SendChat(player, "Error unbanning user [" + Payload + "]");
        ...
        */
        ...
        case HashCode("ban"):
        {
          ...
          if (m_GameLoaded)
          {
            ...
            if (Victim == User)
            {
              SendChat(player, "You cann't ban yourself!");
              break;
            }
            if (Matches == 0)
            ...
            {
              m_Aura->m_DB->BanAddLocal(LastMatch->GetName(), User, Reason);
              SendAllChat("Player [" + LastMatch->GetName() + "] was banned by player [" + User + "]");
            }
            else
        ...
        */
        ...
        case HashCode("checkban"):
        {
          ...
          CDBBan* Ban = m_Aura->m_DB->BanCheckLocal(m_OwnerName, Payload);

          if (Ban)
          {
            SendAllChat("User [" + Payload + "] was banned on [" + Ban->GetDate() + "] by [" + Ban->GetAdmin() + "]");
            delete Ban;
          }
          else
            SendAllChat("User [" + Payload + "] is not banned");
  ...
  switch (CommandHash)
  {
    ...
    case HashCode("ping"):
    {
      //SendChat(player, "Checked player [" + User + "]. Ping: " + (player->GetNumPings() > 0 ? to_string(player->GetPing(m_Aura->m_LCPings)) + "ms" : "X") + ", From: " + m_Aura->m_DB->FromCheck(ByteArrayToUInt32(player->GetExternalIP(), true)) + ", Admin: " + (AdminCheck || RootAdminCheck ? "Yes" : "No") + ", Owner: " + (IsOwner(User) ? "Yes" : "No") + ", Spoof Checked: " + (player->GetSpoofed() ? "Yes" : "No") + ", Realm: " + (player->GetJoinedRealm().empty() ? "LAN" : player->GetJoinedRealm()) + ", Reserved: " + (player->GetReserved() ? "Yes" : "No"));
      SendChat(player, "" + User + ": " + (player->GetNumPings() > 0 ? to_string(player->GetPing(m_Aura->m_LCPings)) + "ms" : "X"));
...
void CGame::EventGameLoaded()
{
  ...
  if (Shortest && Longest)
  {
    SendAllChat("MobaZ Viet Nam chuc cac ban choi game vui ve!");
