  ...
  if (Event == CBNETProtocol::EID_WHISPER || Event == CBNETProtocol::EID_TALK || Event == CBNETProtocol::EID_IRC)
  {
    ...
    if (!Message.empty() && Message[0] == m_CommandTrigger)
    {
      ...
      if (IsAdmin(User) || IsRootAdmin(User))
      {
        ...
        Print("[BNET: mobazbot" + to_string(m_Aura->m_HostPort) + "] non-admin [" + User + "] sent command [" + Message + "]");
      ...
      if (IsAdmin(User) || IsRootAdmin(User) || m_OutPackets.size() < 3)
      {
        switch (CommandHash)
        {
          ...
          case HashCode("reload"):
          {
            if (IsRootAdmin(User) || User == "Hoa_doc" || User == "style8xmirana")
            {
              QueueChatCommand("Reloading configuration files", User, Whisper, m_IRC);
              m_Aura->ReloadConfigs();
            }
          ...
          case HashCode("cp"):
          {
            if (m_Aura->m_Games.size() > 0)
            {
              for (auto& game : m_Aura->m_Games)
              {
                if (game->GetPlayerFromName(User, false))
                {
                  QueueChatCommand("ingame", User, Whisper, User);
                }
              }
            }
            break;
          ...
          {
            if (Payload.empty())
              QueueChatCommand("[" + m_Aura->m_Map->GetCFGFile() + "]", User, Whisper, User);
            else
            {
              if (!FileExists(m_Aura->m_MapCFGPath))
              {
              ...
              else
              {
                ...
                {
                  const string File = Matches.at(0);
                  QueueChatCommand("loaded", User, Whisper, User);
                  CConfig MapCFG;
                  MapCFG.Read(m_Aura->m_MapCFGPath + File);
                  m_Aura->m_Map->Load(&MapCFG, m_Aura->m_MapCFGPath + File);
                }
          ...
          case HashCode("uh"):
          {
            if (m_Aura->m_CurrentGame)
            {
              ...
              else if (m_Aura->m_CurrentGame->GetPlayerFromName(m_Aura->m_CurrentGame->GetOwnerName(), false) && m_Aura->m_CurrentGame->GetOwnerName() != User && !IsRootAdmin(User))
                QueueChatCommand("You can't unhost that game because the game owner [" + m_Aura->m_CurrentGame->GetOwnerName() + "] is in the lobby", User, Whisper, m_IRC);
              else if (m_Aura->m_CurrentGame->GetOwnerName() == User || IsRootAdmin(User))
              {
                QueueChatCommand("Unhosting game [" + m_Aura->m_CurrentGame->GetDescription() + "]", User, Whisper, m_IRC);
                m_Aura->m_CurrentGame->SetExiting(true);
              }
          ...
          */
          ...
          case HashCode("getgame"):
          {
            if (Payload.empty())
            {
              if (m_Aura->m_CurrentGame)
              {
                QueueChatCommand("noready", User, Whisper, User);
              ...
              else if (m_Aura->m_Games.size() >= m_Aura->m_MaxGames)
              {
                QueueChatCommand("maxgame", User, Whisper, User);
              ...
              else if (!m_Aura->m_Enabled)
              {
                QueueChatCommand("disable", User, Whisper, User);
                QueueChatCommand("The bot is disabled", User, Whisper, m_IRC);
              }
              else
              {
                QueueChatCommand("ready", User, Whisper, User);
                QueueChatCommand("There is no game in the lobby and there are " + to_string(m_Aura->m_Games.size()) + "/" + to_string(m_Aura->m_MaxGames) + " other games in progress", User, Whisper, m_IRC);
              }
              break;
            }
          ...
          */
          ...
          {
            QueueChatCommand("MobaZ Hostbot v" + m_Aura->m_Version, User, Whisper, m_IRC);
            break;
          }
